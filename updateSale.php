...
<html>
<head> 
<title>Edit a sales record</title>
<meta charset="utf-8">
	...
	<div class="container">
		...
		<tbody>
			<?php
				...
				$table = mysqli_query($edit, 'Select * FROM invoice i 
				Join invoicedetail id ON i.InvoiceID = id.invoiceID
				Join item it ON id.ItemID = it.ItemID'
				);
				while($row = mysqli_fetch_array($table)){ ?>
					<tr id="<?php echo $row['InvoiceID'];?>">
						...
						<td data-target="TotalPrice"><?php echo $row['Total']; ?></td>
			...
			<div class="modal-content">
			  ...
			  <div class="modal-body">
				...
				<div class="form-group">
					...
					<input type="number" step=".01" id="TotalPrice" class="form-control">
				</div>
			  </div>
...
<script>
	$(document).ready(function(){
		$(document).on('click', 'a[data-role=edit]', function() {
			...
			var ItemPrice = $('#'+InvoiceID).children('td[data-target=ItemPrice]').text();
			var Quantity = $('#'+InvoiceID).children('td[data-target=Quantity]').text();
			var TotalPrice = $('#'+InvoiceID).children('td[data-target=TotalPrice]').text();
			...
			$('#save').click(function() {
				...
				$.ajax({
					url : 'editSale.php',
					method : 'post',
					data : {InvoiceID:InvoiceID, InvoiceDate:InvoiceDate, ItemID:ItemID, ItemName:ItemName, ItemPrice:ItemPrice, Quantity:Quantity, TotalPrice:TotalPrice},
					success : function(response) {
						...
						$('#'+InvoiceID).children('td[data-target=ItemPrice]').text(ItemPrice);
						$('#'+InvoiceID).children('td[data-target=Quantity]').text(Quantity);
						$('#'+InvoiceID).children('td[data-target=TotalPrice]').text(TotalPrice);
						$('#myModal').modal('toggle');
					}
				});
			});
